...
AgregarVariables  <- function( dataset )
{
  ...
  # FACTORES COMPORTAMIENTO DEL CLIENTE
  
  #diferencio clientes que tengan más de seis meses de antigüedad (para hayan contratado por algo específico)
  dataset[  , cliente_antiguedad_cat := cliente_antiguedad ]
  dataset[ cliente_antiguedad<=6 , cliente_antiguedad_cat := 1 ]
  dataset[ cliente_antiguedad>6 , cliente_antiguedad_cat := 0 ]
  
  #diferencio clientes que tengan menos de 25 años (como promedio de edad de emancipación)
  dataset[  , cliente_edad_cat := cliente_edad ]
  dataset[ cliente_edad<=25 , cliente_edad_cat := 1 ]
  dataset[ cliente_edad>25 , cliente_edad_cat := 0 ]
  
  #diferencio clientes que tienen poca cantidad de productos / obligaciones
  dataset[  , cproductos_cat := cproductos ]
  dataset[ cproductos<=3 , cproductos_cat := 1 ]
  dataset[ cproductos>3 , cproductos_cat := 0 ]
  
  #diferencio clientes con mínimo movimiento en caja de ahorro pesos 
  dataset[  , mcaja_ahorro_cat := mcaja_ahorro ]
  dataset[ mcaja_ahorro<=1000 , mcaja_ahorro_cat := 1 ]
  dataset[ mcaja_ahorro>1000 , mcaja_ahorro_cat := 0 ]
  
  #diferencio clientes que no tienen producto esencial tarjeta de débito
  dataset[  , ctarjeta_debito_cat := ctarjeta_debito ]
  dataset[ ctarjeta_debito==0 , ctarjeta_debito_cat := 1 ]
  dataset[ ctarjeta_debito!=0 , ctarjeta_debito_cat := 0 ]
  
  #diferencio clientes grandes inversores
  dataset[  , cplazo_fijo_cat := cplazo_fijo ]
  dataset[ cplazo_fijo>=30 , cplazo_fijo_cat := 1 ]
  dataset[ cplazo_fijo<30 , cplazo_fijo_cat := 0 ]
  
  # FACTORES ALTA CORRELACIÓN
  
  dataset[ , cpaytrx_mpresper:= cpayroll_trx / mprestamos_personales ]
  dataset[ , cpaytrx_mtavicon:= cpayroll_trx / mtarjeta_visa_consumo ]
  dataset[ , ctrxq_mpresper:= ctrx_quarter / mprestamos_personales ]
  dataset[ , ctrxq_Mmlimco:= ctrx_quarter / Master_mlimitecompra ]
  dataset[ , mpresper_Mmlimco:= mprestamos_personales / Master_mlimitecompra ]
  dataset[ , mcajah_mpasma:= mcaja_ahorro / mpasivos_margen ]
  
  # FACTORES BAJA CORRELACIÓN
  
  dataset[ , ccomot_mcuesa:= ccomisiones_otras / mcuentas_saldo ]
  dataset[ , cpaytrx_mcuesa:= cpayroll_trx / mcuentas_saldo ]
  dataset[ , mpasal_mpresper:= mpasivos_margen / mprestamos_personales ]
  dataset[ , ctrxq_mpayroll:= ctrx_quarter / mpayroll ]
  dataset[ , ctrxq_mpasmar:= ctrx_quarter / mpasivos_margen ]
  
  # FACTORES ALTA CORRELACIÓN (2)
  
  dataset[ , macmar_cpaytrx:= mactivos_margen / cpayroll_trx ]
  dataset[ , cmoat_cpaytrx:= cmobile_app_trx / cpayroll_trx ]
  dataset[ , mpresper_mcajah:= mprestamos_personales / mcaja_ahorro ]
  dataset[ , Mfultc_cmoat:= Master_fultimo_cierre / cmobile_app_trx ]
  dataset[ , Mfultc_cpaytrx:= Master_fultimo_cierre / cpayroll_trx ]
  dataset[ , macmar_mcuesa:= mactivos_margen / mcuentas_saldo ]
  
  # FACTORES BAJA CORRELACIÓN (2)
  
  dataset[ , mcuesa_mcapah:= mcuentas_saldo / mcaja_ahorro ]
  dataset[ , mpresper_mcuesa:= mprestamos_personales / mcuentas_saldo ]
  dataset[ , Vifulc_micuesa:= Visa_fultimo_cierre / mcuentas_saldo ]
  dataset[ , ctrxq_mtavicon:= ctrx_quarter / mtarjeta_visa_consumo ]
